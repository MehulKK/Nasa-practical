...
abstract class BaseRecyclerAdapter<T> : RecyclerView.Adapter<BaseRecyclerAdapter<T>.RecyclerHolder>(), Filterable {
    ...
    private val itemTypeImage = 3
    private val itemTypeAudio = 4
    private val itemTypeStory = 5
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerHolder {
        ...
        } else if (viewType == itemTypeImage){
            DataBindingUtil.inflate(LayoutInflater.from(parent.context), getLayoutIdForImage(viewType), parent, false)
        } else if (viewType == itemTypeAudio){
            DataBindingUtil.inflate(LayoutInflater.from(parent.context), getLayoutIdForAudio(viewType), parent, false)
        } else if (viewType == itemTypeStory){
            DataBindingUtil.inflate(LayoutInflater.from(parent.context), getLayoutIdForStory(viewType), parent, false)
        } else {
    ...
    else if (isItemAudio(position))
        itemTypeAudio
    else if (isItemImage(position))
        itemTypeImage
    else if (isItemStory(position))
        itemTypeStory
    else {
        itemTypeNormal
    }
    ...
    /**
     * This fun is used to get layout for audio.
     * @param viewType Int
     * @return Int
     */
    open fun getLayoutIdForAudio(viewType: Int): Int = 0

    /**
     * This fun is used to get layout for Image.
     * @param viewType Int
     * @return Int
     */
    open fun getLayoutIdForImage(viewType: Int): Int = 0

    /**
     * This fun is used to get layout for Story.
     * @param viewType Int
     * @return Int
     */
    open fun getLayoutIdForStory(viewType: Int): Int = 0
    ...
    open fun isItemImage(position: Int) : Boolean = false

    open fun isItemAudio(position: Int) : Boolean = false

    open fun isItemStory(position: Int) : Boolean = false
